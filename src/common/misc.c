...
char *
quoteChars(char *buf, const char *str, const char *qlist)
{
	char *p = buf;
 ...
 *****************************************************************************/
char *
tclizeString(char *buf, const char *str)
{
	int needQuotes = 0;
	int hasQuotes = 0;
	char *p = buf;
...
void
strreplace(char *s, const char *old_str, const char *new_str, int gbl)
{
	char *b = s;
 ...
 *****************************************************************************/
int
getNextToken(char **text, char **token_begin, char **token_end)
{
	...
	for (;;) {
		...
		if (!*p)
			return ST_END;	/* Nothing left to parse */
...
char *
getLineStart(char *p, char *buf_start)
{
	if (p > buf_start)
		p--;
 ...
 *****************************************************************************/
int
isVerilogConstant(const char *s)
{
	int did_digits = 0;
	...
	switch (*s++) {
	case 'd':
		while (isdigit(*s) || strchr("xXzZ", *s) != 0) {
			s++;
			did_digits = 1;
		}
		break;
	case 'h':
		while (isxdigit(*s) || strchr("xXzZ", *s) != 0) {
			s++;
			did_digits = 1;
		}
		break;
	case 'o':
		while ((*s >= '0' && *s <= '7') || strchr("xXzZ", *s) != 0) {
			s++;
			did_digits = 1;
		}
		break;
	case 'b':
		while (strchr("01xzXZ", *s)) {
			s++;
			did_digits = 1;
		}
		break;
