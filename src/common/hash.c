 ...
 */
static void *
h_malloc(int size, char *name)
{
	return malloc(size);
}

static void *
h_calloc(int n, int size, char *name)
{
...
static HashElem *
new_SHashElem(Hash * H, const char *key, unsigned hcode, void *val)
{
...
static HashElem *
new_NHashElem(Hash * H, intptr_t key, unsigned hcode, void *val)
{
	HashElem *E = (HashElem *) HT_MALLOC(H, sizeof(HashElem), "HashElem");
...
void
SHashElem_uninit(HashElem * E, Hash * H)
{
	HT_FREE(H, E->key.s);
}
...
void
delete_Hash(Hash * H, HashElemDelFunc * hdel)
{
	Hash_uninit(H, hdel);
	HT_FREE(H, H);
}
...
Hash_init(Hash * H, int use_ob)
{
	...
	H->loop_ok = 0;
	H->elems =
		(HashElem **) HT_CALLOC(H, H->size, sizeof(HashElem *),
					"HashElem[]");
	if (use_ob)
		ob_touch(H->elems);
}
...
void
Hash_uninit(Hash * H, HashElemDelFunc * hdel)
{
	int i;
 ...
 */
void
Hash_resize(Hash * H, int reqSize)
{
	int S = H->size;
	HashElem **nea;
	int i;
	...
	H->size = reqSize;
	H->mask = H->size - 1;
	nea = (HashElem **) HT_CALLOC(H, H->size, sizeof(HashElem *),
				      "HashElem[]");
...
void
Hash_removeElem(Hash * H, int A, HashElem * E)
{
	HashElem *P;
...
void *
SHash_find(Hash * H, const char *key)
{
...
int
SHash_insert(Hash * H, const char *key, void *val)
{
	unsigned HC = computestrhash(key);
	unsigned A = HC & H->mask;
	HashElem *E;
...
int
SHash_replace(Hash * H, const char *key, void *val)
{
	unsigned HC = computestrhash(key);
	unsigned A = HC & H->mask;
	HashElem *E;
...
int
SHash_remove(Hash * H, const char *key)
{
	unsigned HC = computestrhash(key);
	unsigned A = HC & H->mask;
	HashElem *E;
...
void
Hash_flush(Hash * H, HashElemDelFunc * hdel)
{
	int i;
...
void *
NHash_find(Hash * H, intptr_t key)
{
...
int
NHash_insert(Hash * H, intptr_t key, void *val)
{
	unsigned HC = inthash(key);
	unsigned A = HC & H->mask;
	HashElem *E;
...
int
NHash_replace(Hash * H, intptr_t key, void *val)
{
	unsigned HC = inthash(key);
	unsigned A = HC & H->mask;
	HashElem *E;
...
int
NHash_remove(Hash * H, intptr_t key)
{
	unsigned HC = inthash(key);
	unsigned A = HC & H->mask;
	HashElem *E;
